# Clean Narration Draft

Working target: 14-16 minutes.

This is the human-readable narration draft. Keep this version clean and natural. Add TTS tags only in `script_tts.md` after the wording feels stable.

## Slide 1. Programming In The Language Of Intent

I want to talk about a practice I am converging on for working with AI agents.
...
AI agents have become very good at local code generation.
...
The idea starts from literate programming.
...
Classic literate programming has a tangle step.
...
This means the source does not have to look the same everywhere.
...
So the core definition is simple.
...
Here is a small example: refund eligibility.
...
Another useful pattern is separating boilerplate from logic.
...
When the agent performs the tangle, it turns the human-first source into implementation code, unit tests, integration details, and supporting boilerplate.
...
This is an important point: ALP is not bureaucracy for the human.
...
The reason I am converging on this practice is that it gives me speed without throwing away explanation.
...
ALP is not tied to a specific IDE.
...
The advisor strategy is another practical layer.
...
Once you have a human-first source of intent, project memory becomes much more valuable.
...
At this point, the natural question is: is this just more docs?

It can be, if the docs are passive.
...
Another fair question is: does this slow you down?

Sometimes, yes, at the beginning of a change. But the bet is that it saves time when the project gets revisited, when another agent enters the loop, or when a rule needs to change without breaking the original intent.
...
Of course, not every script or experiment needs this treatment.
...
So this is the practice I am converging on.
